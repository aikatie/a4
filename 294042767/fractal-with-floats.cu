...
__constant__ float xMid;
__constant__ float yMid;
__constant__ int height;
__constant__ int width;
__constant__ float aspect_ratio;
...
void compute_fractal(unsigned char *pic, int num_frames) {
  ...
  const float delta = Delta*pow(0.98, frame);
...
int main(int argc, char *argv[]) {
  double start, end; // TIMER DEPENDS ON DOUBLE
  ...
  /* compute frames */
  const float hst_aspect_ratio = (double)hst_width/hst_height;

  cudaMemcpy(dev_pic, pic, sz, cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(aspect_ratio, &hst_aspect_ratio, sizeof(float));
  cudaMemcpyToSymbol(height, &hst_height, sizeof(int));
  cudaMemcpyToSymbol(width, &hst_width, sizeof(int));

  static const float hst_Delta = 0.001;
  static const float hst_xMid =  0.23701;
  static const float hst_yMid =  0.521;

  cudaMemcpyToSymbol(Delta, &hst_Delta, sizeof(float));
  cudaMemcpyToSymbol(xMid, &hst_xMid, sizeof(float));
  cudaMemcpyToSymbol(yMid, &hst_yMid, sizeof(float));
  ...
  /* end time */
  GET_TIME(end);
  double elapsed = end - start; // TIMER DEPENDS ON DOUBLE
  printf("Cuda compute time: %.4f s\n", elapsed);
  /* write frames to BMP files */
  if ((hst_width <= 320) && (num_frames <= 100)) { /* do not write if images large or many */
