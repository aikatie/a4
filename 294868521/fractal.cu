#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <cuda_runtime.h> // Include CUDA runtime header
#include "timer.h"
#include "fractal.h"

// Constants
static const double Delta = 0.001;
static const double xMid = 0.23701;
static const double yMid = 0.521;

// CUDA kernel to compute fractal
__global__ void computeFractalKernel(
    unsigned char *pic, 
    int width, 
    int height, 
    int num_frames, 
    double delta_base
) {
    // Calculate thread indices
    int frame = blockIdx.z; // 3D grid for frames
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    if (row >= height || col >= width || frame >= num_frames) return;

    // Calculate delta for the current frame (closed-form)
    double delta = delta_base * pow(0.98, frame);

    // Calculate coordinates
    double aspect_ratio = (double)width / height;
    double x0 = xMid - delta * aspect_ratio;
    double y0 = yMid - delta;
    double dx = 2.0 * delta * aspect_ratio / width;
    double dy = 2.0 * delta / height;

    double cx = x0 + col * dx;
    double cy = y0 + row * dy;

    // Compute fractal depth
    double x = cx;
    double y = cy;
    int depth = 256;
    double x2, y2;

    do {
        x2 = x * x;
        y2 = y * y;
        y = 2 * x * y + cy;
        x = x2 - y2 + cx;
        depth--;
    } while ((depth > 0) && ((x2 + y2) < 5.0));

    // Store result in the array
    pic[frame * height * width + row * width + col] = (unsigned char)depth;
}

int main(int argc, char *argv[]) {
    double start, end, start1, end1;

    printf("Fractal v1.6 [CUDA]\n");

    // Read command line arguments
    if (argc != 4) {
        fprintf(stderr, "usage: %s height width num_frames\n", argv[0]);
        exit(-1);
    }
    int width = atoi(argv[1]);
    if (width < 10) {
        fprintf(stderr, "error: width must be at least 10\n");
        exit(-1);
    }
    int height = atoi(argv[2]);
    if (height < 10) {
        fprintf(stderr, "error: height must be at least 10\n");
        exit(-1);
    }
    int num_frames = atoi(argv[3]);
    if (num_frames < 1) {
        fprintf(stderr, "error: num_frames must be at least 1\n");
        exit(-1);
    }
    printf("Computing %d frames of %d by %d fractal\n", num_frames, width, height);

    // Allocate host and device memory
    size_t size = num_frames * height * width * sizeof(unsigned char);
    unsigned char *pic = (unsigned char *)malloc(size);
    unsigned char *d_pic;
    cudaMalloc(&d_pic, size);

    // Start timer
    GET_TIME(start);
    GET_TIME(start1);

    // Define CUDA thread/block configuration
    dim3 threadsPerBlock(16, 16, 1);
    dim3 numBlocks(
        (width + threadsPerBlock.x - 1) / threadsPerBlock.x,
        (height + threadsPerBlock.y - 1) / threadsPerBlock.y,
        num_frames
    );

    // Launch CUDA kernel
    computeFractalKernel<<<numBlocks, threadsPerBlock>>>(d_pic, width, height, num_frames, Delta);

    GET_TIME(end);
    double elapsed = end - start;
    printf("CUDA compute time: %f s\n", elapsed);

    // Copy results back to host
    cudaMemcpy(pic, d_pic, size, cudaMemcpyDeviceToHost);

    GET_TIME(end1);
    double elapsed1 = end1 - start1;
    printf("CUDA compute time (w/ memcpy): %f s\n", elapsed1);

    // Write frames to BMP files
    if ((width <= 320) && (num_frames <= 100)) { /* do not write if images large or many */
        for (int frame = 0; frame < num_frames; frame++) {
            char name[32];
            sprintf(name, "fractal%d.bmp", frame + 1000);
            writeBMP(width, height, &pic[frame * height * width], name);
        }
    }

    // Free memory
    free(pic);
    cudaFree(d_pic);

    return 0;
}
